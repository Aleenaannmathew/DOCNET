...
const DoctorNotifications = () => {
  ...
  useEffect(() => {
    doctorAxios.get('doctor-notifications/')
      .then(res => {
        const updated = res.data.map((notification) => ({
          ...notification,
  ...
  useEffect(() => {
    ...
    ws.onmessage = (e) => {
      ...
      const newNotification = {
        id: data.id || Date.now(),  // Prefer server-side ID
        notification_type: data.notification_type,
        title: `New ${data.notification_type.replace('_', ' ')} Notification`,
            ...
            {notifications.map((notification, index) => {
              ...
              return (
                <div
                  ...
                  className={`flex items-start p-4 rounded-lg border transition shadow-sm ${notification.isRead ? 'border-gray-300' : 'border-emerald-400 bg-emerald-50'}`}
                >
                  <div
                    className={`flex items-center justify-center w-12 h-12 rounded-full ${notification.colorClass} mr-4`}
                  >
